...
pub const Client = struct {
    ...
    fn nodeId(self: *Client, comment: url.Comment) ![]u8 {
        const path = switch (comment.kind) {
            .issuecomment => try std.fmt.allocPrint(
                self.allocator,
                "repos/{s}/{s}/issues/comments/{s}",
                .{ comment.owner, comment.repo, comment.id },
            ),
            .discussion_r => try std.fmt.allocPrint(
                self.allocator,
        ...
        defer parsed.deinit();
        return self.allocator.dupe(u8, parsed.value.node_id);
    }
    ...
    fn minimize(self: *Client, id: []const u8, reason: cli.Reason) ![]u8 {
        ...
        return self.allocator.dupe(u8, minimized.minimizedReason);
    }

    fn expectSuccess(self: *Client, response: http.Response) !void {
        if (response.status.class() != .client_error and
            response.status.class() != .server_error)
        {
            return;
        }
        try self.stderr.print(
            "error: GitHub API returned HTTP {d}: {s}\n",
