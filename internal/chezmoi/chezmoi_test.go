...
func TestExtensionIDsFromStringArrayTrimAndSkipEmpty(t *testing.T) {
	path := filepath.Join(t.TempDir(), "extensions.toml")
	if err := os.WriteFile(
		path,
		[]byte(
			"extensions = [\"one.alpha\", \"\", \" Two.Beta \"]\n",
		),
		0o644,
	); err != nil {
		t.Fatal(err)
...
func TestExtensionIDsFromTableArrayTrimAndSkipEmpty(t *testing.T) {
	path := filepath.Join(t.TempDir(), "extensions.toml")
	if err := os.WriteFile(
		path,
		[]byte(
