...
type environment struct {
	HomeDir             string `env:"CHEZMOI_HOME_DIR"`
	SourceDir           string `env:"CHEZMOI_SOURCE_DIR"`
	OS                  string `env:"CHEZMOI_OS"`
	RaycastAvatarSource string `env:"RAYCAST_AVATAR_SRC"`
}
...
func ContextWithOptions(options Options) (Context, error) {
	...
	homeDir := firstNonEmpty(options.HomeDir, environment.HomeDir, home)
	sourceDir := firstNonEmpty(options.SourceDir, environment.SourceDir)
	if sourceDir == "" {
...
func OSWithOptions(options Options) OSName {
	...
	name := firstNonEmpty(options.OS, environment.OS, runtime.GOOS)
	switch name {
	case "darwin", "macos":
		return Darwin
	case "linux":
